...
Activity界面件传递复杂数据需要用到对象，针对复杂数据直接使用json，采用类似EventBus的做法，数据类型按照项目接口规范走即可(一套接口规范搞定多处数据问题)，以便维护   
...
Android开发存在着众多版本的不同，比如`compileSdkVersion`、`minSdkVersion`、`targetSdkVersion`以及项目中依赖第三方库的版本，不同的module及不同的开发人员都有不同的版本，所以需要一个统一版本规范的文件。</br>
**依赖配置包configLibrary中统一控制，包括各种依赖库** </br>
**SVN或git  提交代码请先更新再提交，如有冲突必须解决冲突 后提交，如将错误代码提交导致他人工作停滞，通报批评！！**</br>
 Ps.自己的错误自行承担，强化责任意识！*公司不听理由，只看结果！*</br>
### 6 第三方库规范
...
* **代码写完后发布之前务必先用findBugs插件查找有误低级错误，然后交由测试人员测试后，才可将apk交与客户**
// TODO...
...
6. *务必保证一个原则：打开一个类最多拖动一到两屏，可以清晰知道界面相关逻辑*</br>
